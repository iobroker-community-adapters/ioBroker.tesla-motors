...
function normalizeTelemetryFieldOptions(fieldName, rawOptions) {
  ...
  if (rawOptions === true || rawOptions === undefined) {
    return { interval_seconds: getDefaultTelemetryInterval(fieldName) };
  }

  if (typeof rawOptions === 'number' || typeof rawOptions === 'string') {
    return { interval_seconds: normalizeTelemetryInterval(rawOptions, fieldName) };
  }
 ...
 *
 * @returns {Record<string, { interval_seconds: number }>}
 */
