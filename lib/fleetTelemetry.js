...
const DEFAULT_TELEMETRY_FIELD_MINIMUM_DELTAS = {
  Soc: 1,
  BatteryLevel: 1,
  ChargeLimitSoc: 1,
...
const TELEMETRY_MINIMUM_DELTA_RULES = [
  { pattern: /(?:Soc|BatteryLevel|Percent|LimitSoc|PedalPosition|BrakePedalPos)$/i, minimumDelta: 1 },
  { pattern: /(?:Temp|Temperature)/i, minimumDelta: 0.5 },
  { pattern: /Pressure/i, minimumDelta: 0.1 },
  { pattern: /(?:Voltage|VBat|BrickVoltage)/i, minimumDelta: 0.1 },
  { pattern: /(?:Current|Amps)/i, minimumDelta: 1 },
  { pattern: /(?:PowerKW|Power|KW)/i, minimumDelta: 0.1 },
  { pattern: /(?:Energy|Remaining)/i, minimumDelta: 0.1 },
  { pattern: /(?:Range|Odometer|Miles|Mile|Mph|Speed)/i, minimumDelta: 1 },
  { pattern: /Heading/i, minimumDelta: 1 },
  { pattern: /Acceleration/i, minimumDelta: 0.1 },
  { pattern: /(?:Duration|Minutes|Delay)/i, minimumDelta: 1 },
  { pattern: /Hours/i, minimumDelta: 0.1 },
  { pattern: /(?:Count|Qty|Distance|FanSpeed|Phases|Torque|Position|Level)$/i, minimumDelta: 1 },
];
const TELEMETRY_MINIMUM_DELTA_EXCLUDED_FIELDS = new Set([
  'DCDCEnable',
  'ChargeEnableRequest',
  'FastChargerPresent',
  'BatteryHeaterOn',
...
const TELEMETRY_MINIMUM_DELTA_EXCLUDED_PATTERNS = [
...
const TELEMETRY_FIELD_GROUP_CONFIG_KEYS = [
...
const TELEMETRY_FIELD_MAPPINGS = {
  ...
  // Fleet Telemetry's ChargeState contains the low-level charger state machine
  // (e.g. Idle, Startup, QualifyLineConfig, Enable). Keep it separate so it
  // does not overwrite the legacy vehicle_data charging_state used by scripts.
  ChargeState: [{ statePath: 'charge_state.telemetry_charge_state', transform: normalizeChargeState, targetType: 'string' }],
  // DetailedChargeState matches the legacy vehicle_data charging_state semantics
  // (Charging, Stopped, Complete, Disconnected, ...). Write it to the existing
  // path and also keep a dedicated detailed state for diagnostics.
  DetailedChargeState: [
    { statePath: 'charge_state.charging_state', transform: normalizeDetailedChargeState, targetType: 'string' },
    { statePath: 'charge_state.detailed_charge_state', transform: normalizeDetailedChargeState, targetType: 'string' },
  ],
...
/**
 * Parses the jsonConfig table representation used by the Admin UI. The table
 * keeps one row per known Fleet Telemetry field so users can enable/disable
 * fields and tune interval/minimum_delta without editing JSON manually.
 *
 * @param {Array<Record<string, any>> | null | undefined} rows
 * @returns {Record<string, { interval_seconds: number } & Record<string, any>> | null}
 */
function parseTelemetryFieldsTableConfig(rows) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== 'object' || row.enabled !== true) {
      continue;
    }

    const fieldName = String(row.field || row.fieldId || '').trim();
    if (!fieldName) {
      continue;
    }

    const fieldOptions = {};
    fieldOptions.interval_seconds = row.interval_seconds === undefined || row.interval_seconds === '' ? getDefaultTelemetryInterval(fieldName) : row.interval_seconds;
    if (row.minimum_delta !== undefined && row.minimum_delta !== '' && row.minimum_delta !== null && row.minimum_delta !== false) {
      fieldOptions.minimum_delta = row.minimum_delta;
    }
    entries[fieldName] = fieldOptions;
  }

  return normalizeTelemetryFieldEntries(entries);
}

/**
 * Parses the category-split jsonConfig tables. Splitting the 200+ Tesla fields
 * into smaller tables keeps the admin page responsive while preserving the
 * same runtime format used for the vehicle configuration.
 *
 * @param {Record<string, any> | null | undefined} config
 * @returns {Record<string, { interval_seconds: number } & Record<string, any>> | null}
 */
function parseTelemetryFieldsGroupedTableConfig(config) {
  if (!config || typeof config !== 'object') {
    return null;
  }

  const rows = [];
  for (const key of TELEMETRY_FIELD_GROUP_CONFIG_KEYS) {
    if (Array.isArray(config[key]) && config[key].length > 0) {
      rows.push(...config[key]);
    }
  }

  return rows.length > 0 ? parseTelemetryFieldsTableConfig(rows) : null;
}

/**
 * Resolves the Fleet Telemetry field configuration from adapter native data.
 * The grouped jsonConfig tables have priority once users save the Admin UI.
 * The old single-table and JSON settings remain supported for existing
 * installations and experts.
 *
 * @param {Record<string, any>} config
 * @returns {Record<string, { interval_seconds: number } & Record<string, any>>}
 */
function parseTelemetryFieldsFromAdapterConfig(config) {
  const groupedTableFields = parseTelemetryFieldsGroupedTableConfig(config);
  if (groupedTableFields) {
    return groupedTableFields;
  }

  const tableFields = parseTelemetryFieldsTableConfig(config && config.telemetryFields);
  if (tableFields) {
    return tableFields;
  }
  return parseTelemetryFieldsConfig(config && config.telemetryFieldsJson);
}
...
function normalizeTelemetryFieldOptions(fieldName, rawOptions) {
  ...
  if (normalized.minimum_delta === undefined) {
    return addDefaultTelemetryMinimumDelta(fieldName, normalized);
  }
...
function getDefaultTelemetryMinimumDelta(fieldName) {
  const explicitMinimumDelta = DEFAULT_TELEMETRY_FIELD_MINIMUM_DELTAS[fieldName] ?? DEFAULT_TELEMETRY_FIELDS[fieldName]?.minimum_delta;
  if (explicitMinimumDelta !== undefined) {
    return explicitMinimumDelta;
  }
  if (TELEMETRY_MINIMUM_DELTA_EXCLUDED_FIELDS.has(fieldName)) {
    return undefined;
  }
  if (TELEMETRY_MINIMUM_DELTA_EXCLUDED_PATTERNS.some((pattern) => pattern.test(fieldName))) {
    return undefined;
  }
  const rule = TELEMETRY_MINIMUM_DELTA_RULES.find((entry) => entry.pattern.test(fieldName));
  return rule ? rule.minimumDelta : undefined;
}
...
class FleetTelemetryManager {
  ...
  constructor(adapter) {
    ...
    this.pendingStateValues = new Map();
    this.lastStateTimestamps = new Map();
  }
  ...
  async start() {
    ...
    this.client.on('reconnect', () => {
      this.adapter.log.debug('Reconnecting to telemetry MQTT broker');
    });
  ...
  async handleMessage(topicBase, topic, payload) {
    ...
    if (parsedTopic.kind === 'metric' && parsedTopic.vin && parsedTopic.fieldName) {
      ...
      if (updates.length) {
        for (const update of updates) {
          ...
          await this.setStateIfChanged(update.id, update.value);
        }
      } else {
        ...
        if (rawUpdate) {
          ...
          await this.setStateIfChanged(rawUpdate.id, rawUpdate.value);
        }
      }
      return;
    }
    ...
    if (parsedTopic.kind === 'connectivity' && parsedTopic.vin) {
      const connectivityValue = JSON.stringify(decoded);
      await this.ensureStateExists(`${parsedTopic.vin}.telemetry.connectivity`, connectivityValue, 'json');
      await this.setStateIfChanged(`${parsedTopic.vin}.telemetry.connectivity`, connectivityValue);
      return;
    }
    ...
    if ((parsedTopic.kind === 'error' || parsedTopic.kind === 'alert') && parsedTopic.vin) {
      ...
      const eventValue = JSON.stringify(decoded);
      await this.ensureStateExists(`${parsedTopic.vin}.telemetry.${suffix}`, eventValue, 'json');
      await this.setStateIfChanged(`${parsedTopic.vin}.telemetry.${suffix}`, eventValue);
    }
  }

  /**
   * Writes telemetry states only when the value really changed. Fleet Telemetry
   * can replay retained MQTT values on connect while the periodic API sync is
   * also updating the same legacy states. Avoiding duplicate identical writes
   * prevents SQL history batches from containing the same id/timestamp pair
   * twice when several unchanged values arrive in the same millisecond.
   *
   * @param {string} id
   * @param {any} value
   * @returns {Promise<boolean>} true when a state write was performed
   */
  async setStateIfChanged(id, value) {
    if (this.hasPendingStateValue(id, value)) {
      return false;
    }

    try {
      const currentState = await this.adapter.getStateAsync(id);
      if (currentState && areStateValuesEqual(currentState.val, value)) {
        return false;
      }
    } catch (error) {
      // A missing state can happen before ensureStateExists has fully populated
      // the object cache. In that case we still want to write the value below.
      this.adapter.log.debug(`Could not read current telemetry state ${id}: ${error.message}`);
    }
    ...
    this.pendingStateValues.set(id, value);
    try {
      await this.adapter.setStateAsync(id, {
        val: value,
        ack: true,
        ts: this.getNextStateTimestamp(id),
      });
      return true;
    } finally {
      if (areStateValuesEqual(this.pendingStateValues.get(id), value)) {
        this.pendingStateValues.delete(id);
      }
    }
  }
   ...
   * @returns {boolean}
   */
  hasPendingStateValue(id, value) {
    return this.pendingStateValues.has(id) && areStateValuesEqual(this.pendingStateValues.get(id), value);
  }

  /**
   * ioBroker's SQL adapter uses state id + millisecond timestamp as primary
   * key. Fleet Telemetry can deliver several changed values for the same field
   * within one millisecond on reconnect. Use a per-state monotonic timestamp so
   * every real telemetry value remains historized without duplicate-key errors.
   *
   * @param {string} id
   * @returns {number}
   */
  getNextStateTimestamp(id) {
    const now = Date.now();
    const lastTimestamp = this.lastStateTimestamps.get(id) || 0;
    const nextTimestamp = now > lastTimestamp ? now : lastTimestamp + 1;
    this.lastStateTimestamps.set(id, nextTimestamp);
...
module.exports = {
  ...
  TELEMETRY_FIELD_GROUP_CONFIG_KEYS,
