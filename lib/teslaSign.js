...
class TeslaCommandSigner {
  ...
  async autoSeatClimate(seatPosition, on) {
    // Tesla's signed command protobuf currently supports automatic seat
    // climate for the two front seats. ioBroker uses 0/1 to keep the command
    // naming aligned with remote_seat_heater_request.
    const seatMap = {
      0: 1, // AutoSeatPosition_FrontLeft
      1: 2, // AutoSeatPosition_FrontRight
    };
    const seatPositionValue = seatMap[seatPosition];
    if (!seatPositionValue) {
      throw new Error(`Unsupported auto seat climate position: ${seatPosition}`);
    }

    return this._executeCarServerAction({
      autoSeatClimateAction: {
        carseat: [{ on: !!on, seatPosition: seatPositionValue }],
      },
    });
  }
